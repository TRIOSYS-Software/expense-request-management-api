...
type AdvanceApprovals struct {
	ID           uint       `json:"id,omitempty" gorm:"primaryKey;autoIncrement;unique"`
	RequestID    uint       `json:"request_id,omitempty" gorm:"not null;index:idx_advance_approvals_request_level,priority:1"`
	ApproverID   uint       `json:"approver_id,omitempty" gorm:"not null;index"`
	Level        uint       `json:"level,omitempty" gorm:"not null;index:idx_advance_approvals_request_level,priority:2"`
